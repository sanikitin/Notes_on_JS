...
<ol>
  ...
  <li><a href="https://github.com/sanikitin/Notes_on_JS#%D0%BC%D0%B5%D1%82%D0%BE%D0%B4%D1%8B-%D0%BE%D0%B1%D1%8A%D0%B5%D0%BA%D1%82%D0%B0-this">Методы объекта, "this"</a></li>
</ol>
...
- Общепринятые руководства по стилю кода
...
ВАЖНО!: Один тест проверяет одну вещь. Принципиальная разница в том, что когда один из ```assert``` выбрасывает ошибку, то выполнение ```it``` блока тут же прекращается. Таким образом, если первый ```assert``` выбросит ошибку, результат работы второго ```assert``` мы уже не узнаем.
...
Проверка существования объекта. При обращении к свойству, которого нет, возвращается undefined. Это позволяет просто проверить существование свойства:
```javascript
let user = {};
...
2. Операторы равенства ```==``` и строгого равенства ```===``` для объектов работают одинаково. Два объекта равны только в том случае, если это один и тот же объект.
...
Можно реализовать глубокое клонирование, используя рекурсию. Или, чтобы не изобретать велосипед, использовать готовую реализацию — метод ```_.cloneDeep(obj)``` из JavaScript-библиотеки lodash.
...
<h3><b>Методы объекта, "this"</b></h3>

Функцию, которая является свойством объекта, называют методом этого объекта. Пример: 
```javascript
let user = {
  // ...
};
...
Для доступа к информации внутри объекта метод может использовать ключевое слово ```this```. Значение ```this``` – это объект «перед точкой», который использовался для вызова метода. Пример: 
```javascript
let user = {
  name: "Джон",
  age: 30,
...
ВАЖНО!: ```«this»``` не является фиксированным. Правило: при вызове ```obj.f()``` значение ```this``` внутри ```f``` равно ```obj```.
ВАЖНО!: Вызов без объекта: ```this == undefined```. Обычно подобный вызов является ошибкой программирования.
...
У стрелочных функций нет «this». Если используется this внутри стрелочной функции, то его значение берётся из внешней «нормальной» функции. Например:
```javascript
...
user.sayHi(); // Илья
```
Это полезно, когда не нужно иметь отдельное значение this, а нужно брать его из внешнего контекста.

Внутренняя реализация: Ссылочный тип или почему ```this``` не всегда работает подробно далее:
<details>
...
```
В последней строчке кода используется условный оператор ```?```, который определяет, какой будет вызван метод ```(user.hi или user.bye)``` в зависимости от выполнения условия. В данном случае будет выбран ```user.hi```. Затем метод тут же вызывается с помощью скобок ```()```. Но вызов не работает как положено!

При вызове будет ошибка, потому что значением ```"this"``` внутри функции становится ```undefined```.

Так работает (доступ к методу объекта через точку): ```user.hi();```
Так уже не работает (вызываемый метод вычисляется): ```(user.name == "Джон" ? user.hi : user.bye)(); // Ошибка!```

Как работает вызов методов ```(obj.method())```.
...
Как передаётся информация о ```this``` из первой части во вторую?

Если поместить эти операции в отдельные строки, то значение ```this```, естественно, будет потеряно:
```javascript
let user = {
  name: "Джон",
  hi() { alert(this.name); }
...
Этот ссылочный тип является внутренним типом. Мы не можем явно использовать его, но он используется внутри языка. Значение ссылочного типа – это «триплет»: комбинация из трёх значений (base, name, strict), где:
